...
function peer_cert() {
    ...
    mkdir -p "organizations/peerOrganizations/$ORG.workspase.id/ca"
    mkdir -p "organizations/peerOrganizations/$ORG.workspase.id/msp/cacerts"
    mkdir -p "organizations/peerOrganizations/$ORG.workspase.id/msp/tlscacerts"
    mkdir -p "organizations/peerOrganizations/$ORG.workspase.id/peers"
    mkdir -p "organizations/peerOrganizations/$ORG.workspase.id/tlsca"

    CERT_DIR=organizations/peerOrganizations/$ORG.workspase.id

    if [ ! -f "$CERT_DIR/ca/ca-key.pem" ]; then

        cfssl gencert -initca "${PWD}/organizations/cfssl/ca-peer.json" | cfssljson -bare "$CERT_DIR/ca/ca"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/tlsca/tlsca.$ORG.workspase.id-cert.pem"
        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/ca/ca.$ORG.workspase.id-cert.pem"
...
function orderer_cert() {
    TYPE=$1 #orderer user
    USER=$2 #orderer.workspase.id

    mkdir -p organizations/ordererOrganizations/workspase.id/ca
    mkdir -p organizations/ordererOrganizations/workspase.id/msp/cacerts
    mkdir -p organizations/ordererOrganizations/workspase.id/msp/tlscacerts
    mkdir -p organizations/ordererOrganizations/workspase.id/orderers
    mkdir -p organizations/ordererOrganizations/workspase.id/tlsca

    CERT_DIR=organizations/ordererOrganizations/workspase.id

    if [ ! -f "$CERT_DIR/ca/ca-key.pem" ]; then

        cfssl gencert -initca "${PWD}/organizations/cfssl/ca-orderer.json" | cfssljson -bare "$CERT_DIR/ca/ca"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/tlsca/tlsca.workspase.id-cert.pem"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/msp/cacerts/"
        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/msp/tlscacerts/tlsca.workspase.id-cert.pem"
...
function generate_orderer_certs() {
    ...
    for DIR in cacerts keystore signcerts tlscacerts; do
        mkdir -p "organizations/ordererOrganizations/workspase.id/orderers/$USER/msp/$DIR"
    done

    mkdir -p "organizations/ordererOrganizations/workspase.id/orderers/$USER/tls"
    ...
    cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/orderers/$USER/msp/tlscacerts/tlsca.workspase.id-cert.pem"
